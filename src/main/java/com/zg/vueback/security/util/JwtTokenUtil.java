...
import com.zg.vueback.cache.RedisCache;
    ...
    public Boolean validateTokenAndRefresh(String token, String username) {
        ...
        if(StringUtils.isNotBlank(cacheToken)){
            if(!validateToken(cacheToken)){
                ...
                redisCache.set("token_" + token, newCacheToken,this.expiration * 2 / 1000);
            }
            return true;
        }
        return false;
        ...
        if(StringUtils.isNotBlank(token)){
            ...
            if(StringUtils.isNotBlank(cacheToken)){
                if(validateToken(token)){
                    ...
                    redisCache.set("token_" + token, cacheToken,this.expiration * 2 / 1000);
